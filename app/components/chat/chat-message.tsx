    ...
    <div className="flex gap-2">
      {message.experimental_attachments
        ...
        .map((attachment, index) => (
          <a key={`${message.id}-${index}`} href={attachment.url} className="rounded-md object-contain w-64 h-64" />
        ))}
    </div>
  );
  ...
  const renderThinkingProcess = () =>
    ...
    message.role === 'assistant' && (
      <details className="mb-2 text-sm" open>
        <summary className="cursor-pointer text-muted-foreground hover:text-foreground">Thinking process</summary>
